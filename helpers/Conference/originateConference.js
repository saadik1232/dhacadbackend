          ...
          for (let i = 0; i < ENDPOINT.length; i++) {
              ...
              outgoing.once("StasisEnd", function (event, channel) {
                bridge.destroy(function (err) {
                  if (err)
                    ...
                    console.log(
                      `Call Ended: ${JSON.stringify(event, null, 2)}`
                    );
                });
              });
            ...
            outgoing.originate(
              ...
              function (err, channel) {
                if (err)
                  console.log(
                    `Call Origination Error: ( ${ENDPOINT[i]} ) ${err}`
                  );
                else {
                  // setTimeout(() => {
                  //   bridge.destroy(function (err) {
                  //     if (err) console.log(`Call Ending Error: `, err);
                  //     else console.log(`Call Ended: `);
                  //   });
                  // }, 5000);
                  console.log(`Call Origination Successfull: `);
                }
              }
            );
          }
