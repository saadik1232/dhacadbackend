...
module.exports.fetchAll = async (req, res, next) => {
  var user = await User.findAll({ where: { roleId: 2 } });
  if (user) {
    logs("Supervisor", "Fetched All");
...
module.exports.filter = async (req, res, next) => {
  if (CheckIfEmpty(req.params.id)) {
    var user = await User.findOne({ where: { id: req.params.id, roleId: 2 } });
    if (user) {
      logs(
        " Specific Supervisor Fetched ",
        " Fetching a Specific Supervisor from the Database "
      );
      res.json(RES(200, "Success", user));
